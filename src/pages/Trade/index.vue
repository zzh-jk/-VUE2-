<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in addressList" :key="item.id" @click="changeSelectAddress(item,addressList)">
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b><i>{{orderList.totalNum}}</i>类商品，总商品金额</b>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{userDefaultIno.fullAddress}}</span>
        收货人：<span>{{userDefaultIno.consignee}}</span>
        <span>{{userDefaultIno.phoneNum}}</span>
      </div>
    </div>
...
import { mapState } from 'vuex';
  export default {
    ...
    mounted(){
      this.$store.dispatch('tradeOption/getAddressList');
      this.$store.dispatch('tradeOption/getOrderList');
    },
    computed: {
     ...mapState('tradeOption',['addressList','orderList']),
     userDefaultIno(){
       return this.addressList.find(item=>item.isDefault==1)||{};
     },
     goodsList(){
      return this.orderList.detailArrayList||[];
     }
    },
    methods:{
      //排他选择地址
      changeSelectAddress(item,addressList){
     ...
     async submitOrder(){
        ...
        const {consignee,fullAddress,phoneNum}=this.userDefaultIno;
        let data = {
          consignee,
         ...
         await this.$API.reqSubmitOrder(tradeNo,data).then((result) => {
          if(result.code==200){
            console.log(result);
            let tradeId=result.data
            this.$router.push('/pay?tradeId='+tradeId);
          }
         }).catch((err) => {
          alert(err.message);
         });
