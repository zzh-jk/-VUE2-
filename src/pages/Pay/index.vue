...
export default {
  ...
  methods: {
    // 获取订单信息
    getOrderInfo() {
      this.$API.reqCreateNative(this.tradeId).then((res) => {
        this.codeUrl = res.data.codeUrl;
      });
    },
    //ElementUI弹出框
    async open() {
      await QRCode.toDataURL(this.codeUrl)
        .then((url) => {
          console.log(url);
          this.url = url;
        })
      ...
      this.$alert(`<img src=${this.url} alt="二维码" />`, "微信支付", {
        center: true,
        ...
        beforeClose:(action,instance,done)=>{
          if(action === 'cancel'){
            alert('支付遇到问题，请联系管理员：13700000000');
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          await this.$API.reqQueryPayStatus(this.tradeId).then((res) => {
            this.code  = res.code;  //保存二维码状态
            if (res.code == 200) {
              clearInterval(this.timer);
              this.timer = null;
              this.$msgbox.close();
